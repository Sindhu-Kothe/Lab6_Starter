...
function initFormHandler() {
  ...
  formElement.addEventListener('submit', event => {
    ...
    // B5. Create an empty object and extract the form data into it
    const recipeObject = {};
    ...
    // B9. Get the recipes array from localStorage, add the new recipe, and save it back
    let recipes = JSON.parse(localStorage.getItem('recipes')) || [];
    recipes.push(recipeObject);
    localStorage.setItem('recipes', JSON.stringify(recipes));
    
    formElement.reset(); // Reset the form fields after submission
  });

  // B10. Get a reference to the "Clear Local Storage" button
  const clearButton = document.querySelector('button[type="button"].danger');

  // B11. Add a click event listener to the clear local storage button
